...
NUM_OPERATORS = 15
CONTEXT_DIM = 160       # fuzzing context vector dim: 16x10 (placeholder, will be replaced later)
...
class OperatorPolicyNet(nn.Module):
    ...
    def __init__(self, context_dim, num_ops, op_embed_dim, hidden_dim):
        ...
        self.net = nn.Sequential(
            nn.Linear(context_dim + op_embed_dim, hidden_dim),
            nn.ReLU(),
...
def run(args):
    ...
    # print(f"  ops:      {ops.shape}")
    # print(f"  rewards:  {rewards.shape}")
    
    # print(f"\nFirst 20 samples:")
    # print(f"{'Index':<8} {'Operator':<10} {'Reward':<10}")
    # print("-" * 30)
    # for i in range(min(20, len(ops))):
    #     print(f"{i:<8} {ops[i]:<10} {rewards[i]:<10.2f}")
    ...
    #     print(f"  Operator {op_id:2d}: {count:6d} ({percentage:5.2f}%)")
    ...
    # print(f"\nSample context vectors (first 3, first 5 dimensions):")
    # for i in range(min(3, len(contexts))):
    #     print(f"  Sample {i}: {contexts[i][:5]}...")
    ...
    model = OperatorPolicyNet(
        context_dim=CONTEXT_DIM,      # 160 (16x10)
    ...
    # Save embeddings
    embed_path = os.path.join(args.out_dir, "operator_embeddings.npy")
    np.save(embed_path, final_embeds)
    print(f"✓ Saved operator embeddings to: {embed_path}")
    
    # Save similarity matrix
    sim_path = os.path.join(args.out_dir, "operator_similarity_matrix.npy")
